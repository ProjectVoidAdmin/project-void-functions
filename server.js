...
class CombatRoom {
  ...
  _projectMember(m) {
    return { uid:m.uid, name:m.name, h:m.hp, mx:m.maxHp, en:m.energy, al:m.alive?1:0 };
  }
  _projectEnemy(e) {
  ...
  async _endCombat(outcome) {
    ...
    this.broadcast({
      ...
      goldEach,
      kills,
      // Include leader's respawnZone so clients can teleport to the correct safe zone on death
      respawnZone: outcome === "death" ? (this.members[0]?.respawnZone || "duskwatch") : undefined,
      members: this.members.map(m => this._projectMember(m)),
      enemies: this.enemies.map(e => this._projectEnemy(e)),
    });
