...
function flagAnomaly(uid, reason, details) {
  ...
  if (_anomalyLog.length > 1000) _anomalyLog.shift();
  ...
  ws.on("message", async (raw) => {
    ...
    switch (msg.type) {
      ...
      case "admin_get_flags": {
        ...
        try {
          const snap = await db.ref("pv/anomalies").limitToLast(100).get();
          const data = snap.exists() ? snap.val() : {};
          console.log(`[ADMIN] get_flags returning ${Object.keys(data).length} entries`);
      ...
      case "admin_reset_gold": {
        if (!clientUid || !ADMIN_UIDS.has(clientUid)) return;
        const { targetUid: rgUid, amount } = msg;
        if (!rgUid) return;
        try {
          await db.ref(`pv/saves/${rgUid}/player/gold`).set(amount || 0);
          send(ws, { type:"admin_action_ok", action:"reset_gold", targetUid: rgUid });
          console.log(`[ADMIN] reset_gold uid=${rgUid} amount=${amount||0} by=${clientUid}`);
        } catch (e) {
          send(ws, { type:"admin_action_fail", action:"reset_gold", reason: e.message });
        }
        break;
      }

      case "admin_wipe_inventory": {
        if (!clientUid || !ADMIN_UIDS.has(clientUid)) return;
        const { targetUid: wiUid } = msg;
        if (!wiUid) return;
        try {
          await db.ref(`pv/saves/${wiUid}/player/inventory`).set({ weapons:[], armor:[], accessories:[], provisions:[], materials:[] });
          await db.ref(`pv/saves/${wiUid}/player/equipment`).set({ weapon:null, armor:null, accessories:[null,null,null] });
          send(ws, { type:"admin_action_ok", action:"wipe_inventory", targetUid: wiUid });
          console.log(`[ADMIN] wipe_inventory uid=${wiUid} by=${clientUid}`);
        } catch (e) {
          send(ws, { type:"admin_action_fail", action:"wipe_inventory", reason: e.message });
        }
        break;
      }
      ...
      case "admin_wipe_actions": {
        if (!clientUid || !ADMIN_UIDS.has(clientUid)) return;
        const { targetUid: waUid } = msg;
        if (!waUid) return;
        try {
          await db.ref(`pv/saves/${waUid}/player/learnedActions`).set(["basic_attack"]);
          await db.ref(`pv/saves/${waUid}/player/actionSlots`).set(["basic_attack",null,null,null,"provisions","flee"]);
          send(ws, { type:"admin_action_ok", action:"wipe_actions", targetUid: waUid });
          console.log(`[ADMIN] wipe_actions uid=${waUid} by=${clientUid}`);
        } catch (e) {
          send(ws, { type:"admin_action_fail", action:"wipe_actions", reason: e.message });
        }
        break;
      }
      ...
      case "admin_get_banned": {
        ...
        try {
          ...
          if (savesSnap.exists()) {
            ...
            for (const [uid, save] of Object.entries(saves)) {
              if (save.banned === true) {
                const name = save.player?.name || null;
                // Look up username from accounts
                let username = null;
                const accSnap = await db.ref("pv/accounts").orderByChild("uid").equalTo(uid).limitToFirst(1).get();
                if (accSnap.exists()) { username = Object.keys(accSnap.val())[0]; }
                players.push({ uid, name, username, bannedAt: save.bannedAt || null });
              }
            }
          }
          send(ws, { type:"admin_banned_list", players });
          console.log(`[ADMIN] get_banned found=${players.length} by=${clientUid}`);
        } catch (e) {
          send(ws, { type:"admin_banned_list", players: [] });
        }
        break;
